...
export default function CargarVisitaModal({
  isOpen,
  onClose,
  socio,
  onSuccess,
}: CargarVisitaModalProps) {
  ...
  const cargarCostoVisita = async () => {
    try {
      ...
      if (error) {
        ...
        // Si no hay configuración, usar valor por defecto según especificación
        // Según especificación: 15% de Cuota Social = $4,200
        setCostoUnitario(4200);
      }
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Cargar Visita" size="md">
              ...
              >
                ...
                {socios.map((s) => (
                  <option key={s.id} value={s.id}>
                    {s.numero_socio} - {getNombreCompleto(s)}
                  </option>
                ))}
              </select>
            </div>
