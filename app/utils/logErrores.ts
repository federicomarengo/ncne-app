...
export function logError(
  ...
  try {
    ...
    logger.error(`[Error Log] ${tipo}:`, mensaje, error);
  } catch (err) {
    // Si falla el logging, al menos loggear en consola
    logger.error('Error al guardar log de error:', err);
    logger.error('Error original:', tipo, mensaje, error);
  }
}
...
export function obtenerLogs(): ErrorLog[] {
  try {
    ...
    logger.error('Error al leer logs:', err);
    return [];
  }
}
...
export function limpiarLogs(): void {
  try {
    ...
    logger.error('Error al limpiar logs:', err);
  }
}
